...
| C-16c | 預約 步驟1 日期時段人數 | 基礎 | `/reserve/when` |
...
- 兩張卡片，各自帶自己的按鈕：
  - **用會員訂**（建議）：姓名電話自動帶入、可自己改期取消、當天用餐自動累點 → C-12
  - **直接訂，不註冊**：留姓名與手機就好，改期取消要打電話 → C-16b
...
- 頂部步驟指示 `① 選時間 ─ ② 先點餐 ─ ③ 完成`
- **C-16c**：月曆（今天 +1 至 +30 天）→ 時段按鈕（5 個，客滿的顯示「已額滿」且不可點）→ 人數步進器 → 顯示「將為您安排 4 人桌」
...
| S-02b | 開桌設定人數 | 基礎 | `/t/:tableNo/open` |
