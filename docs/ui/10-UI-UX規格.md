...
>    那張空桌在桌況上就顯示成「**預約保留**」，不能直接開給別人；櫃檯**隨時**可以取消保留（還沒進保留窗也能先取消）。
>    **候位仍然不鎖桌。**
>
...
- 這一列兩顆按鈕：
  ...
  - 「**取消保留**」（secondary）：先跳二次確認「**取消後，這筆訂位到時候不會鎖住 A08，而且無法恢復。客人到店時再幫他選別張桌。**」，
...
- **取消保留隨時可以按，不限保留窗內**：後端前置條件只有 `status = CONFIRMED` 且 `hold_released_at IS NULL`；
  已經取消過再按回 `409 HOLD_ALREADY_RELEASED`（前端顯示並重新載入）。
  從桌況點進來的一定是保留窗內；**還沒進保留窗的訂位要先取消，到 S-07 那一列按**（例如知道客人會晚很久，想把桌先開給現場）
...
| 情況 | 狀態膠囊 | 操作欄 |
|---|---|---|
| `CONFIRMED`，離用餐時間還超過 30 分鐘（還沒取消保留） | 已確認 | 沒有報到按鈕，灰字「**18:00 起可報到**」（用餐時間 −30 分）；ghost「**取消保留**」 |
| `CONFIRMED`，在報到時間窗內（−30 ～ +10 分），配到的桌保留中（`holding = true`） | 已確認＋小字「**保留 A08**」 | primary「**報到開桌**」→ S-07b 選一張空桌（A08 排第一）；ghost「**取消保留**」 |
| 同上，但配到的桌還在用餐中或待清理（沒有保留） | 已確認 | primary「報到開桌」→ S-07b；ghost「取消保留」 |
| `CONFIRMED`，櫃檯已經取消保留（`holdReleasedAt` 有值），不論時間 | 已確認＋小字「**已取消保留**」 | 沒有「取消保留」；報到按鈕照上面的時間規則（窗內 primary「報到開桌」→ S-07b，更早是灰字） |
...
- **「取消保留」隨時可以按**：凡是 `CONFIRMED`、有配桌、還沒取消過保留（`holdReleasedAt` 是 `null`）的列都有這顆 ghost，
  **不限保留窗內**（還沒到的訂位也能先取消，到時候那張桌就不會顯示預約保留）。重複按回 `409 HOLD_ALREADY_RELEASED`
- 跟 S-02d 同一個二次確認（「**取消後，這筆訂位到時候不會鎖住 A08，而且無法恢復。客人到店時再幫他選別張桌。**」）、同一支 `release-hold`；
