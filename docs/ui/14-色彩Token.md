...
`docs/` 裡的 Mermaid 圖（流程圖、時序圖、架構圖）有自己一組 `classDef` 配色，
其中 `#EFE9E1`（灰米色，用在「停止／安靜／模擬」的節點）**不在 App 色票裡**。

那是給**看文件的人**用的，不是給畫面用的，所以不進 `tokens.colors.css`，
也不要拿去當 UI 的顏色。圖表配色維持跟品牌色系一致即可，不需要納管。
