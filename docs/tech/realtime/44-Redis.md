...
**難度** ★★★☆☆　**用在哪些模組** M8，以及進階的快取與鎖　**哪幾週** 第 3 週
...
- **不能掉的東西**（訂單、會員、帳單，還有整桌共用的購物車）→ MySQL
- **掉了也還好、但要很快的東西**（驗證碼、快取）→ Redis

> **購物車為什麼不放 Redis？** 送出點餐時，購物車要跟寫單、扣庫存在**同一個資料庫交易**裡搬進 `order_ticket` 再清掉。
> 放在兩個地方就沒辦法一起成功、一起失敗（見 [交易 Transaction](../database/27-交易Transaction.md)），所以放 MySQL 最單純。
