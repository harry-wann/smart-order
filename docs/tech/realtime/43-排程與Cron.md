...
> 現在線上訂位**還是會鎖住配到的桌**（M7 的 7.10「訂位保留桌位」），但**不用排程**：查桌況的時候，看這張桌有沒有一筆 `CONFIRMED` 訂位落在「開始前 30 分～開始後 10 分」、而且沒被櫃檯取消保留，有就顯示「預約保留」。
...
**⑦ 用排程去改「算得出來」的狀態**
例如到時間把桌子改成「預約保留」。排程晚跑幾分鐘、伺服器剛好重開、櫃檯取消保留後又被排程改回去——桌況就跟訂位對不上。
