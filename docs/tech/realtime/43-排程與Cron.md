...
**難度** ★★☆☆☆　**用在哪些模組** M2、M5、M7、M8　**哪幾週** 第 4～5 週
...
> 原本還有第十支「桌位預留轉換」（時間到了把桌位欄位改成 `RESERVED`），已經整條刪掉，所以是九支。
> 現在線上訂位**還是會鎖住配到的桌**（M7 的 7.10「訂位保留桌位」），但**不用排程**：查桌況的時候，看這張桌有沒有一筆 `CONFIRMED` 訂位落在「開始前 30 分～開始後 10 分」，有就顯示「預約保留」；要解開就是取消那筆訂位（訂位轉 `CANCELLED`，條件自然不成立）。
> 桌位欄位本身仍然只存 `AVAILABLE`／`OCCUPIED`／`CLEANING`。寫法見 [區間重疊與訂位排程](../advanced/46-區間重疊與訂位排程.md)。
...
> 清單只列用餐時間在「現在 −10 分～+30 分」之間的（也就是開始前 30 分到開始後 10 分）；
> 超過用餐時間 10 分鐘報到回 `409 RESERVATION_EXPIRED`（請改登記候位），還沒到開始前 30 分就報到回 `409 RESERVATION_TOO_EARLY`。不能只靠排程。
...
**⑦ 用排程去改「算得出來」的狀態**
例如到時間把桌子改成「預約保留」。排程晚跑幾分鐘、伺服器剛好重開、訂位已經取消了桌子卻還鎖著——桌況就跟訂位對不上。
→ 能從現有資料當場算出來的（訂位保留），就在查詢時算，不要存、也不要排程同步。
